...
async function main() {
    ...
    // -----------------------------------------------------------------------
    // 2. Generate Global Insights
    // -----------------------------------------------------------------------
    const globalPrompt = `
As a lead crypto market strategist for FinixHub, provide a comprehensive yet accessible analysis of the current market state for ${targetDate}.

CURRENT DATE: ${targetDate} (Ensure all analysis treats events prior to this date as past).

DATA CONTEXT:
- Market Momentum: ${context.momentumPercent.toFixed(1)}% of top 50 assets are trading above their 50-day SMA.
- 24h Market Cap Change: ${context.marketCapChangePercent.toFixed(2)}%.
- Latest News Headlines: ${JSON.stringify(context.news)}
- Current Fear & Greed Index: ${context.fearGreedIndex || 'N/A'} (Note: If present, use this as the primary sentiment indicator).

Return a JSON object with:
{
    "fear_greed_index": number, // Use ${context.fearGreedIndex} if provided, otherwise estimate based on news/momentum.
    "mood_label": "e.g., Extreme Fear, Calm Recovery, Euphoric",
    "mood_text": "Detailed explanation of market sentiment (Maximum 7 sentences). Use **Markdown** (bold key terms).",
    "momentum_label": "e.g., Strong Bullish, Consolidating",
    "momentum_text": "How the momentum data suggests the market is trending (Maximum 7 sentences). Use **Markdown**.",
    "flow_label": "e.g., Institutional Inflow, Retail Panic",
    "flow_text": "Interpretation of capital movement (Maximum 7 sentences). Use **Markdown**.",
    "top_story": "A punchy summary of the most impactful news event."
}`;

    let globalAIResult: any;
    let globalModelUsed = 'deepseek-chat';

    // Use DeepSeek as primary model (cheapest)
    if (DEEPSEEK_KEY) {
        console.log(`  Global insights → using deepseek-chat...`);
        globalAIResult = await callAI(
            'deepseek-chat',
            [
                { role: 'system', content: 'You are a financial analyst specializing in "Plain English" crypto insights.' },
                { role: 'user', content: globalPrompt },
            ],
            DEEPSEEK_KEY,
            false,
        );
        globalModelUsed = 'deepseek-chat';
    } else if (OPENAI_KEY) {
        console.log('  Global insights → falling back to gpt-4o-mini...');
        globalAIResult = await callAI(
            'gpt-4o-mini',
            [
                { role: 'system', content: 'You are a financial analyst specializing in "Plain English" crypto insights.' },
        ...
        globalModelUsed = 'gpt-4o-mini';
    }

    console.log(`  ✅ Global insights generated (${globalModelUsed})`);

    // Audit log
    await supabase.from('ai_audit_log').insert({
        user_id: null,
        model_used: globalModelUsed,
        input_tokens: globalAIResult.usage?.prompt_tokens,
        output_tokens: globalAIResult.usage?.completion_tokens,
        cost_usd: 0,
        request_type: 'daily_insights_global',
    ...
    const { error: globalError } = await supabase
        ...
        .upsert(
            {
                date: targetDate,
                ...globalAIResult.content,
                momentum_percent: context.momentumPercent,
                market_cap_change: context.marketCapChangePercent,
                citations: globalAIResult.citations,
            },
            { onConflict: 'date' },
        );

    if (globalError) console.error('  ❌ Error saving global insights:', globalError);

    // -----------------------------------------------------------------------
    // 3. Generate Asset Insights
    // -----------------------------------------------------------------------
